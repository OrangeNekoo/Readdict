...
private:
    ...
    static QByteArray statusLine(const MockRequest &mr) {
        ...
        // MKCOL：目录已存在（405）——验证 put() 的 MKCOL 兜底不阻断 PUT
        if (mr.method == "MKCOL")
...
private slots:
    ...
    // 含 RFC1123+GMT、RFC3339 兜底、percent-decode、子目录（尾斜杠）过滤
    void listParsesMultistatus() {
        MockDav server;
        QVERIFY(server.listen(QHostAddress::LocalHost, 0));
        WebDavClient c("http://127.0.0.1:" + QString::number(server.serverPort()) + "/sync", "u", "p");
        const auto res = c.list();
        QCOMPARE(res.size(), 3); // settings.json + notes.txt + my file.json（subdir/ 被滤）
        QVERIFY(res[0].modified.isValid());
